...
const Gauge: React.FC<GaugeProps> = ({ value, label }) => {
  ...
  const progressOffset = circumference - (value / 100) * (circumference * visibleFraction);

  // Dynamic color based on value ranges
  const getColor = (v: number) => {
    if (v < 30) return '#3b82f6'; // Blue-500
    if (v < 60) return '#22c55e'; // Green-500
    if (v < 85) return '#eab308'; // Yellow-500
